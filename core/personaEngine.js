...
class PersonaEngine {
    ...
    async initialize() {
        ...
        const customPersonas = await storage.getItem('customPersonas') || [];
        this.personas = [...BUILTIN_PERSONAS, ...customPersonas];
    ...
    async setActivePersona(id) {
        ...
        if (persona) {
            ...
            await storage.setItem('activePersonaId', id);
            return persona;
        }
        return null;
    }
    ...
    async getActivePersona() {
        if (!this.activePersona) {
            const savedId = await storage.getItem('activePersonaId');
            if (savedId) {
    ...
    async saveCustomPersonas() {
        const customPersonas = this.personas.filter(p => !p.isBuiltin);
        await storage.setItem('customPersonas', customPersonas);
    }
}
