...
import { getItem, setItem, clearAllData } from '../utils/storage.js';
import { exportToTXT, exportToJSON } from '../utils/fileExport.js';
...
class SettingsUI {
    async render() {
        ...
        const autoExport = await getItem('autoExport', 'settings') || false;
            ...
            <div class="settings-container">
                ...
                <div class="settings-section">
                    ...
                    <p style="font-size: 0.85rem; color: var(--text-secondary); margin-top: 0.5rem;">
                        ...
                        ⚠️ Changing model requires reload</p>
                </div>
    ...
    setupEventListeners() {
        ...
        if (modelSelect) {
            ...
            modelSelect.addEventListener('change', async (e) => {
                await setItem('preferredModel', e.target.value, 'settings');
                modal.confirm(
                    'Reload Required',
        ...
        if (autoExportToggle) {
            autoExportToggle.addEventListener('change', async (e) => {
                await setItem('autoExport', e.target.checked, 'settings');
            });
        }

        const exportTxtBtn = document.getElementById('export-txt-btn');
        if (exportTxtBtn) {
            exportTxtBtn.addEventListener('click', () => this.handleExportTXT());
        }
        ...
        if (resetQuotaBtn) {
            resetQuotaBtn.addEventListener('click', () => this.handleResetQuota());
        }
    }
